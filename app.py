...
try:
    ...
    if os.path.exists('model/logistic.pkl'):
        models['logistic'] = joblib.load('model/logistic.pkl')

    if os.path.exists('model/results.pkl'):
        results_file = joblib.load('model/results.pkl')
        for name in models.keys():
            if name in results_file:
                model_results[name] = results_file[name]
            else:
                model_results[name] = {'accuracy': 0.0}
    else:
        for name in models.keys():
            model_results[name] = {'accuracy': 0.0}
...
def predict():
    ...
    try:
        ...
        for model_name, model in models.items():
            ...
            anomaly_percent = (anomaly_count / len(df)) * 100 if len(df) > 0 else 0
            ...
            comparison_results[model_name] = {
                ...
                'accuracy': model_results.get(model_name, {}).get('accuracy', 0.0),
                'avg_anomaly_prob': f"{np.mean(probs):.1%}" if probs is not None else "N/A"
            }
...
def download_report():
    ...
    if not last_comparison_results:
        flash('No report available yet. Please run prediction first.')
        return redirect(url_for('index'))
    ...
    report_df = pd.DataFrame([
        {
            ...
            'Avg Anomaly Probability': result['avg_anomaly_prob']
        }
